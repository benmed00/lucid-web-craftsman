...
- **Deploy Edge (payment return):** **`npm run deploy:functions:payment-success`** — `order-lookup`, `stripe-session-display`, `verify-payment`. **`npm run deploy:functions:stripe-return`** — same plus `send-order-confirmation` and `stripe-webhook`. `supabase/functions/deno.lock` and `supabase/functions/*/deno.lock` are **gitignored** so a Deno 2 lockfile v5 is not committed (the hosted bundler rejects it).
