...
- **Pricing snapshot (checkout):** **`npm run test:pricing-snapshot`** — Deno tests for `stripe-webhook/lib/pricing-snapshot.ts` and `send-order-confirmation/_lib/email-pricing-from-db.ts`, plus Vitest for `src/lib/checkout/pricingSnapshot.ts` (Zod). Same Deno tests run in **Deno create-payment** GitHub Actions (see `.github/workflows/deno-create-payment.yml`). **Contract & versioning rules:** [`supabase/functions/_shared/PRICING_SNAPSHOT.md`](supabase/functions/_shared/PRICING_SNAPSHOT.md) — read before evolving the snapshot shape.
- **Build:** `npm run build`
