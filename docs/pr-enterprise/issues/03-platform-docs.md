...
Ship a **maintained documentation layer** so engineers, reviewers, and agents can answer: *where do rules live?*, *what is client-only vs server-enforced?*, and *what happens in checkout edge cases?* without spelunking the monolith. This issue covers **RULES_REGISTRY**, **BUSINESS_LOGIC_AND_EDGE_CASES**, **TECH_MAP**, **HOOKS**, **DATA_TYPES**, **CLIENT_VS_SERVER_RULES**, and index updates in **docs/README.md** + **AGENTS.md**.
