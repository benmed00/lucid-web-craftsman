...
function stubStore(
  impl: (id: string, opts: { maxAttempts: number; windowMs: number }) =>
...
Deno.test('composite: primary success short-circuits the fallback', async () => {
  let fallbackCalls = 0;
...
Deno.test('composite: primary throw → fallback consulted, onFallback fires', async () => {
