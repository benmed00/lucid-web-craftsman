...
serve(async (req: Request): Promise<Response> => {
  ...
  try {
    ...
    const { data: alerts, error: fetchError } = await supabase.rpc(
    ...
    const alertsHtml = (alerts as SecurityAlert[])
      .map(
        (alert) => `
    ...
    const criticalCount = alerts.filter(
      (a: SecurityAlert) => a.severity === 'critical'
    ).length;
    const highCount = alerts.filter(
      (a: SecurityAlert) => a.severity === 'high'
    ).length;
    ...
    const emailResult = await sendBrevoEmail(
      ADMIN_EMAIL,
      `🚨 [URGENT] ${criticalCount + highCount} Alerte(s) de Sécurité`,
      emailHtml
    );
    ...
    const { error: updateError } = await supabase.rpc('mark_alerts_notified', {
      alert_ids: alertIds,
    });
    if (updateError)
      console.error('[Security Alert] Error marking alerts:', updateError);
