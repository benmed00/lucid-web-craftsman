...
serve(async (req) => {
  ...
  try {
        ...
        if (
          ...
          if (
            ...
            if (
              ...
              logStep('Server-verified discount', {
                code: coupon.code,
    ...
    // ========================================================================
    const subtotalEuros = verifiedItems.reduce(
      (sum, item) => sum + item.product.price * item.quantity,
      0
    );
    ...
    const shippingCentsEstimate =
      subtotalEuros > 0 && !hasFreeShipping ? 695 : 0;
    ...
    if (discountAmountCents > maxAllowedDiscount) {
      logStep('Capping discount to meet Stripe minimum', {
    ...
    const discountRatio =
      discountAmountCents > 0 && subtotalCents > 0
    ...
    const vipThreshold =
      (businessRules?.cart?.highValueThreshold || 1000) * 100;
