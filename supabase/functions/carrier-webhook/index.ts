...
const corsHeaders = {
  ...
  'Access-Control-Allow-Headers':
    'authorization, x-client-info, apikey, content-type, x-signature, x-webhook-id, x-webhook-timestamp',
};
...
function parseCarrierWebhook(
  ...
  switch (carrier.toLowerCase()) {
    case 'dhl': {
      ...
      const shipment = (payload.shipments as unknown[])?.[0] as
        | Record<string, unknown>
        | undefined;
      const event = (shipment?.events as unknown[])?.[0] as
        | Record<string, unknown>
        | undefined;
      ...
      return {
        ...
        location:
          ((event.serviceArea as Record<string, unknown>)
    ...
    case 'colissimo': {
      ...
      return {
        carrier: 'colissimo',
        event_type:
          (evt?.code as string) || (payload.eventCode as string) || 'unknown',
        tracking_number:
          (payload.parcelnumber as string) ||
    ...
    case 'chronopost': {
      return {
        carrier: 'chronopost',
        event_type:
          (payload.eventCode as string) ||
          (payload.code as string) ||
          'unknown',
        tracking_number:
          (payload.trackingNumber as string) ||
          (payload.skybillNumber as string) ||
          '',
        status:
          (payload.eventLabel as string) || (payload.label as string) || '',
...
serve(async (req) => {
  ...
  try {
    ...
    const webhookSecret =
      Deno.env.get(`${carrierUpper}_WEBHOOK_SECRET`) ||
    ...
    if (webhookSecret) {
      ...
      if (
        expectedSignature.length !== sigToCheck.length ||
        expectedSignature !== sigToCheck
      ) {
    ...
    if (!order) {
      logStep('Order not found for tracking number', {
    ...
    if (mapping.createAnomaly) {
      ...
      if (!anomalyError) {
        ...
        await supabaseService
          ...
          .update({
            has_anomaly: true,
            anomaly_count: (order as Record<string, number>).anomaly_count
              ? (order as Record<string, number>).anomaly_count + 1
              : 1,
            requires_attention:
              mapping.anomalySeverity === 'high' ||
              mapping.anomalySeverity === 'critical',
