/**
 * Regenerates platform catalog and fails if catalog.json would change.
...
const SPEC = '.github/project/catalog.json';
...
if (diff.status === 1) {
  console.error(
    `Drift in ${SPEC}: run pnpm run project:catalog and commit .github/project/catalog.json and PROJECT_CATALOG.md.`
  );
  process.exit(1);
}
