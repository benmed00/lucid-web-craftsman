...
describe('Checkout Flow', () => {
  ...
  it('should add a product to the cart', () => {
    ...
    cy.get('[data-testid="add-to-cart"], button').contains(/ajouter|add/i).first().click();
    // Cart icon should show at least 1 item
    cy.get('[data-testid="cart-count"], [aria-label*="cart"], .cart-count')
      .should('exist');
  });

  it('should navigate to checkout with items in cart', () => {
    // Add a product
    cy.get('button').contains(/ajouter|add/i).first().click();
    // Go to cart
    cy.visit('/cart');
    cy.get('body').should('be.visible');
    // Click proceed to checkout
    cy.get('button, a').contains(/commander|checkout|paiement/i).first().click();
  ...
  it('should fill shipping info and advance to payment', () => {
    ...
    // Advance to payment
    cy.get('button').contains(/paiement|payment/i).first().click();
  ...
  it('should show error for invalid promo code', () => {
    ...
    cy.get('input[placeholder*="promo"], input[placeholder*="Code"]').first().type('INVALIDCODE123');
    cy.get('button').contains(/appliquer|apply/i).first().click();
  ...
  it('should clear promo error when user types new code', () => {
    ...
    cy.get('input[placeholder*="promo"], input[placeholder*="Code"]').first().type('BAD');
    cy.get('button').contains(/appliquer|apply/i).first().click();
    ...
    cy.get('input[placeholder*="promo"], input[placeholder*="Code"]').first().clear().type('NEW');
