...
const SERVICE_ROLE_KEY =
  ...
  (import.meta.env as { SUPABASE_SERVICE_ROLE_KEY?: string })
...
describe('verify-payment', () => {
  it('should reject missing session_id', async () => {
    ...
    expect(
      data?.error || data?.success === false || (error as object)
    ).toBeTruthy();
  }, 15000);
...
describe('send-order-confirmation', () => {
  ...
  it('should require Authorization (anon key is not service role)', async () => {
    ...
    expect(errMsg).toMatch(
      /Unauthorized|Invalid token|Forbidden|Missing required|non-2xx/i
    );
  }, 15000);

  it.skipIf(!hasServiceRole)(
    'should return preview when previewOnly=true with service role',
    async () => {
      const { data } = await serviceClient.functions.invoke(
        'send-order-confirmation',
        {
          body: {
  ...
  it.skipIf(!hasServiceRole)(
    ...
    async () => {
      ...
      expect(data?.error).toMatch(
        /Missing required|orderId|customerEmail|customerName/i
      );
    },
    15000
  );
...
describe('send-shipping-notification', () => {
  it.skipIf(!hasServiceRole)(
    'should return preview when previewOnly=true',
    async () => {
      const { data } = await serviceClient.functions.invoke(
        'send-shipping-notification',
        {
...
describe('send-vip-order-notification', () => {
  it.skipIf(!hasServiceRole)(
    'should reject missing payload fields with service role',
    async () => {
      const { data } = await serviceClient.functions.invoke(
        'send-vip-order-notification',
        {
...
describe('send-order-notification-improved', () => {
  it('should require auth (reject anon)', async () => {
    ...
    const errMsg =
      data?.error || (error as { message?: string })?.message || '';
    expect(errMsg).toMatch(
      /Unauthorized|Invalid token|Forbidden|Order not found|non-2xx/i
    );
  }, 15000);
});
...
describe('check-promo-alerts', () => {
  it('should require admin auth', async () => {
    ...
    const errMsg =
      data?.error || (error as { message?: string })?.message || '';
    expect(errMsg).toMatch(
      /Unauthorized|Invalid token|Forbidden|Admin|non-2xx/i
    );
  }, 15000);
});
...
describe('security-alert-notification', () => {
  it.skipIf(!hasServiceRole)(
    'should return success when no pending alerts',
    async () => {
      const { data } = await serviceClient.functions.invoke(
        'security-alert-notification',
        {
      ...
      expect(data?.success).toBe(true);
      expect(
        data?.message === 'No pending alerts' ||
          data?.alertsProcessed !== undefined
      ).toBe(true);
    },
    15000
  );
...
describe('stripe-webhook', () => {
  it.skipIf(!isRealSupabase)(
    'should reject non-POST method',
    async () => {
