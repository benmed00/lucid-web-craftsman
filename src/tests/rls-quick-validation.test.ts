...
describe('RLS Quick Validation - Anonymous Access', () => {
  ...
  describe('Sensitive Tables - Must Block Anonymous SELECT', () => {
    for (const table of SENSITIVE_TABLES) {
      it(`should BLOCK anonymous SELECT from ${table}`, async () => {
        ...
        expect(isBlocked, `Anonymous should NOT be able to read from ${table}`).toBe(true);
      });
    }
  });
  ...
  describe('Sensitive Tables - Must Block Anonymous INSERT', () => {
    const insertTests = [
      { table: 'profiles', data: { id: '00000000-0000-0000-0000-000000000001' } },
      { table: 'admin_users', data: { email: 'hacker@evil.com', name: 'Hacker', user_id: '00000000-0000-0000-0000-000000000001' } },
      { table: 'user_roles', data: { user_id: '00000000-0000-0000-0000-000000000001', role: 'super_admin' } },
      { table: 'audit_logs', data: { action: 'FAKE_LOG', resource_type: 'test' } },
    ...
    for (const { table, data } of insertTests) {
      it(`should BLOCK anonymous INSERT into ${table}`, async () => {
        ...
        expect(error, `Anonymous should NOT be able to insert into ${table}`).not.toBeNull();
      });
    }
  });
