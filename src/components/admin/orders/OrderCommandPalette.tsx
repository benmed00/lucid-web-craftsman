...
export function OrderCommandPalette({
  ...
  const executeAction = async (action: () => Promise<void>, label: string) => {
    ...
    try {
      ...
      toast.error(`Erreur: ${error instanceof Error ? error.message : 'Échec de l\'action'}`);
    } finally {
  ...
  const actions: CommandAction[] = useMemo(() => [
    ...
    },
    {
      ...
      category: 'status',
      disabled: ['delivered', 'cancelled', 'refunded'].includes(currentStatus),
      badge: 'Attention',
    ...
    },
    {
      ...
      action: async () => {
        ...
        const productIds = (data || []).map(i => i.product_id).filter(Boolean) as number[];
        ...
        const stockInfo = (products || []).map(p => {
          const ordered = data?.find(i => i.product_id === p.id)?.quantity || 0;
    ...
    },
    {
      ...
      disabled: ['created', 'payment_pending', 'cancelled', 'refunded'].includes(currentStatus),
      action: async () => {
  ...
  return (
    <CommandDialog open={open} onOpenChange={onOpenChange}>
      <CommandInput placeholder="Rechercher une action... (⌘K)" disabled={isExecuting} />
      <CommandList>
