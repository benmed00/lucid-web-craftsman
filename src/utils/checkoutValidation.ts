...
const DANGEROUS_PATTERNS = ['javascript:', '<script', 'eval(', 'onclick', 'onerror', 'onload', '<img', '<svg', 'data:'];
...
const addressComplementSchema = z.string()
  .optional()
  .transform(val => val?.trim() || '')
  .refine(val => {
    if (!val) return true;
...
export const validatePromoCode = (code: string): {
  ...
  return { success: false, error: result.error.issues[0]?.message || 'Code invalide' };
};
