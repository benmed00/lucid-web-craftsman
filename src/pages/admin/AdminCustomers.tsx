...
const AdminCustomers = () => {
  ...
  const [sortBy, setSortBy] = useState<
    'created_at' | 'total_spent' | 'total_orders'
  >('created_at');
      ...
      const customersWithStats = (profilesRes.data || []).map((profile) => {
        ...
        const cancelledOrders = userOrders.filter((o) =>
          [
            ...
            'payment_failed',
            'delivery_failed',
          ].includes(o.order_status || o.status || '')
        );
  ...
  const openCustomerDetail = useCallback(async (customer: Customer) => {
    ...
    try {
        ...
        if (productIds.length > 0) {
          ...
          cartItems = cartItems.map((c) => ({
            ...
            product_name: c.product_id
              ? productMap.get(c.product_id)?.name || `Produit #${c.product_id}`
              : 'Inconnu',
            product_price: c.product_id
              ? productMap.get(c.product_id)?.price
              : undefined,
          }));
        }
      }
      ...
      setSelectedCustomer((prev) =>
        prev ? { ...prev, orders, cart_items: cartItems } : null
      );
    } catch (e) {
  ...
  const filteredCustomers = useMemo(() => {
    ...
    result.sort((a, b) => {
      ...
      if (sortBy === 'total_orders')
        return b.stats.total_orders - a.stats.total_orders;
      return (
        new Date(b.created_at).getTime() - new Date(a.created_at).getTime()
      );
    });
              ...
              <tbody>
                {paginatedCustomers.map((customer) => (
                  ...
                  >
                    ...
                    <td className="p-4">
                      ...
                      {customer.stats.cancelled_orders > 0 && (
                        <p className="text-xs text-red-400">
                          dont {customer.stats.cancelled_orders} annulée
                          {customer.stats.cancelled_orders > 1 ? 's' : ''}
                        </p>
                      )}
                      <p className="text-xs text-muted-foreground">
                    ...
                    <td className="p-4 hidden lg:table-cell">
                      {customer.loyalty ? (
                        getLoyaltyBadge(customer.loyalty.tier)
                      ) : (
          ...
          {totalPages > 1 && (
            <div className="flex items-center justify-between p-4 border-t border-border">
              <p className="text-sm text-muted-foreground">
                Page {currentPage} sur {totalPages} • {filteredCustomers.length}{' '}
                résultats
              </p>
              ...
              <Tabs defaultValue="info" className="mt-4">
                <TabsList className="grid w-full grid-cols-4">
                  ...
                  <TabsTrigger value="cart">
                    Panier{' '}
                    {selectedCustomer.cart_items &&
                    selectedCustomer.cart_items.length > 0
                        ...
                        <div className="grid grid-cols-3 gap-3">
                          <div className="text-center p-3 bg-muted/50 rounded-lg">
                            <p className="text-xl font-bold text-foreground">
                              {selectedCustomer.loyalty.points_balance}
                            </p>
                            <p className="text-xs text-muted-foreground">
                              Points disponibles
                            </p>
                          ...
                          <div className="text-center p-3 bg-muted/50 rounded-lg">
                            <p className="text-xl font-bold text-foreground">
                              {selectedCustomer.loyalty.total_points_earned}
                            </p>
                            <p className="text-xs text-muted-foreground">
                              Points cumulés
                            </p>
                  ...
                  {detailLoading ? (
                    ...
                    <div className="space-y-2">
                      <p className="text-sm text-muted-foreground mb-3">
                        {selectedCustomer.cart_items.length} article
                        {selectedCustomer.cart_items.length > 1 ? 's' : ''} dans
                        le panier
                      </p>
                  ...
                  <div className="grid grid-cols-2 lg:grid-cols-3 gap-4">
                    ...
                    <div className="text-center p-4 bg-muted/50 rounded-lg">
                      <p className="text-3xl font-bold text-emerald-400">
                        {selectedCustomer.stats.paid_orders}
                      </p>
                      <p className="text-sm text-muted-foreground">
                        Validées / Payées
                      </p>
                    ...
                    <div className="text-center p-4 bg-muted/50 rounded-lg">
                      <p className="text-3xl font-bold text-red-400">
                        {selectedCustomer.stats.cancelled_orders}
                      </p>
                      <p className="text-sm text-muted-foreground">
                        Annulées / Échouées
                      </p>
                    ...
                    <div className="text-center p-4 bg-muted/50 rounded-lg">
                      <p className="text-lg font-bold text-foreground">
                        {selectedCustomer.stats.last_order_date
                          ? format(
                              new Date(selectedCustomer.stats.last_order_date),
                              'dd MMM yyyy',
                              { locale: fr }
                            )
                          : 'Jamais'}
